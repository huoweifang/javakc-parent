...
public class DispOrdService extends BaseService<DispOrdDao,DispOrd> {
     ...
     * @return
     */
    public Page<DispOrd> findPageDispOrd(DispOrdQuery dispOrdQuery,
                                         int pageNum, int pageSize){

        SimpleSpecificationBuilder<DispOrd> simpleSpecificationBuilder =
        ...
        if (!StringUtils.isEmpty(dispOrdQuery.getOrderName())){
             ...
             * !null表示 is not null
             */
            simpleSpecificationBuilder.and("orderName",":",
                    dispOrdQuery.getOrderName());
        }
        Page page = dao.findAll(simpleSpecificationBuilder.getSpecification(),
